...
def get_graph():
    ...
    """
    graph = facebook.GraphAPI(version="7.0")
...
def canv_url(request):
    ...
    if request.is_secure():
        return (
            "https://"
            + request.get_host()
            + reverse("social_login:facebook_login_response")
        )
    else:
        return (
            "https://"
...
def login_successful(code, request):
    ...
    extra_data = graph.get_object(
        str(social_id) + "/?fields=name,first_name,last_name,link"
    )
    ...
    new = FacebookUser.create_or_update(
        social_id,
        access_token,
        token_expires,
        first_name,
