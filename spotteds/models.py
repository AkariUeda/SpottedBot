...
class Spotted(models.Model):
    ...
    author = models.ForeignKey(  # User Obj from author
        User, related_name="authored", blank=True, null=True, on_delete=models.SET_NULL
    )
    target = models.ForeignKey(  # Targeted user
        User, related_name="targeted", blank=True, null=True, on_delete=models.SET_NULL
    )
    ...
    )  # unique post_id to use with Graph
    api_id = models.IntegerField(default=0)  # unique API id
    dismissed = models.BooleanField(
        default=False
    )  # used to save the state of spotteds dismissed by the target
    ...
    )  # When someone reports a spotted, this is the report text
    share_with_crush = models.BooleanField(
        default=True
    )  # Whether or not to let crush know who is the author
    ...
    def remove_spotted(self, db_remove=False):
        ...
        try:
            ...
            if (
                ...
                not in getattr(e, "message", "")
            ):
                logger.exception(
                    "Exception raised while trying to delete spotted(%s) from Facebook",
                    self.post_id,
                )
        if db_remove:
            # delete from db
...
class PendingSpotted(models.Model):
    ...
    def post_spotted(self, mod=None):
        ...
        s = Spotted(
            ...
            public=self.public,
            api_id=self.api_id,
            share_with_crush=self.share_with_crush,
        )
        s.save()
        ...
        try:
            ...
            if not settings.TEST_MODE:
                resp = page_graph().put_object(
                    "me", "feed", message=f_message, link=attachment
                )
            else:
                resp = {"id": 1}
        ...
        # Try to catch invalid url exception
            ...
            # Else, raise the exception and log it
                ...
                logger.exception(
                    "Exception raised while trying to post spotted(%s) to Facebook",
                    self.id,
                )
                raise e
        except Exception as e:
            logger.exception(
                "Exception raised while trying to post spotted(%s) to Facebook", self.id
            )
            raise e
