...
class PendingSpottedForm(forms.ModelForm):
    ...
    target_name = forms.CharField(
        required=False,
        label="Nome do(a) Crush",
        widget=forms.TextInput(
            attrs={"class": "typeahead", "placeholder": "Crush Santos da Silva"}
        ),
    )
    target_id = forms.CharField(required=False, widget=forms.HiddenInput())
    share_with_crush = forms.BooleanField(
        required=False, initial=True, label="Informar crush que você é x autorx"
    )

    if settings.RECAPTCHA_PUBLIC_KEY:
        captcha = ReCaptchaField(
            attrs={
                "callback": "captchaSpottedCallback",
            }
        )
    ...
    class Meta:
        ...
        widgets = {
            "attachment": forms.TextInput(
                attrs={"placeholder": "http://crush.com/vamo_se_pegar.gif"}
            )
        }
        ...
        if target_id:
            try:
                ...
                raise forms.ValidationError(
                    target_name
                    + " ainda não está cadastrado(a). Verifique sua digitação ou deixe em branco."
                )
        return None
    ...
    def save(self, author):
        ...
        try:
            ...
            logger.info(
                "%s\n%s",
                delete_old_spotteds.delete_executer(5, True),
                inspect_database.inspect_executer(),
            )
            instance.save()
        return instance
